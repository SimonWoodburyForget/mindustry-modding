...
*** BulletType
    ...
    | homingPower        | float        |       0 | Doesn't do anything specific; if ~homingPower~ larger then ~0.01~ gets displayed in the UI, if ~homingPower~ is larger then ~0.0001~ it tracks enemies. |
