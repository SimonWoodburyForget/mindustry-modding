...
#+TEXINFO_HEADER: Modding documentation for Mindustry v99 (159)
...
** Mindustry JSON
   ...
   * ~//~ may be used for single line comments;
   * ~//~ can't be used in strings, except if prefixed with ~https:~ / ~http:~;
   * ~"~ aren't required for strings;
   * ~,~ commas aren't required for objects or arrays;
   * ~[ value, ]~ commas can trail within objects or arrays.
...
** FAQ
    ...
    * What's a ~NullPointerException~? This is an error message that indicates a field is null and shouldn't be null, meaning one of the required fields may be missing.
...
*** ItemTurret
    ...
    ItemTurret is a turret that shoots bullets relative to an item type.
