...
** Sound
  ...
  * ~.mp3~ required for iOS
  ...
  Just like any other assets, you reference them by the stem of your filename(s): ~pewpew.ogg~ and ~pewpew.mp3~ is referenced with ~pewpew~ from a field of type ~Sound~.
...
** Zone
    You can add your mod's campaign map by this
    ...
    | conditionWave      | int             | MAX_VALVE  |
    | alwaysUnlocked     | boolean         |            |
    | launchPeriod       | int             | 10         |
    | loadout            | [[Loadout]]     | basicShard |
    | resources          | [[Item]]        |            |
    | rules              | [[Rules]]       |            |
    | requirements       | [[Objective]]   |            |
    | configureObjective | [[Objective]]   |            |
    | defaultStartingItems | [[ItemStack]] |            |
    
    note:
    * The JSON name in ~content/zones/~ must as same as your maps name in ~maps/~ 
    
*** Loadout
    Loadout is the blueprint when your core landed.
    
    In Loadout:
    | code  | means       |
    |-------+-------------|
    | ~>~ | conveyor face to right |
    | ~^~ | conveyor face to up |
    | ~<~ | conveyor face to left |
    | ~v~ | conveyor face to down |
    | ~1~ | coreShard |
    | ~2~ | coreFoundation |
    | ~3~ | coreNucleus |
    | ~#~ | space that occupied by core and drill |
    | ~　~ | air |
    | ~C~ | mechicalDrill with four copper ore |
    
    Note:
    * You must set a core in your custom Loadout
...
*** Generator
    [[Abstract]] type for generate map
    
    | field  | type | note |
    |--------+------+------|
    | width  | int  | [[MapGenerator]] will set it |
    | height | int  | [[MapGenerator]] will set it |
    
**** MapGenerator
    Extends [[Generator]] -an useful map generator
    
    | field        | type           | default | notes |
    |--------------+----------------+---------+-------|
    | map          | Map            |         | this is set in map editor |
    | mapName      | String         |         | |
    | decorations  | [[Decoration]] |         | |
    | enemySpawns  | int            | -1      | The amount of final enemy spawns used. -1 to use everything in the map.This amount of enemy spawns is selected randomly from the map. |
    | distortFloor | boolean        | false   | Whether floor is distorted along with blocks. |
    
***** Decoration
    A method in [[MapGenerator]] -Decoration is a random generator to generate some block in floor you set.
...
* Game
  ...
  :END:
** Rules
    Rules is applied to map,it can be edited at map editor,you can edit your mod-added campaign map's rule through these fields.
    
    | field                    | type          | default    | notes       |
    |--------------------------+---------------+------------+-------------|
    | infiniteResources        | boolean       |            | Whether the player has infinite resources. |
    | waveTimer                | boolean       | true       | Whether the waves come automatically on a timer. If not, waves come when the play button is pressed. |
    | waves                    | boolean       |            | Whether waves are spawnable at all. |
    | enemyCheat               | boolean       |            | Whether the enemy AI has infinite resources in most of their buildings and turrets. |
    | pvp                      | boolean       |            | Whether the game objective is PvP. Note that this enables automatic hosting. |
    | unitDrop                 | boolean       | true       | Whether enemy units drop random items on death. |
    | unitBuildSpeedMultiplier | float         | 1          | How fast unit pads build units. |
    | unitHealthMultiplier     | float         | 1          | How much health units start with. |
    | playerHealthMultiplier   | float         | 1          | How much health players start with. |
    | playerDamageMultiplier   | float         | 1          | How much damage player mechs deal. |
    | unitDamageMultiplier     | float         | 1          | How much damage any other units deal. |
    | buildCostMultiplier      | float         | 1          | Multiplier for buildings for the player. |
    | buildSpeedMultiplier     | float         | 1          | Multiplier for building speed. |
    | enemyCoreBuildRadius     | float         | 400        | No-build zone around enemy core radius. |
    | dropZoneRadius           | float         | 300        | Radius around enemy wave drop zones. |
    | respawnTime              | float         | 60 * 4     | Player respawn time in ticks. |
    | waveSpacing              | float         | 60 * 60 * 2 | Time between waves in ticks. |
    | bossWaveMultiplier       | float         | 3          | How many times longer a boss wave takes. |
    | launchWaveMultiplier     | float         | 2          | How many times longer a launch wave takes. |
    | zone                     | [[Zone]]      |            | Zone for saves that have them. |
    | spawns                   | [[SpawnGroup]] |            | Enemy spawn layout. |
    | limitedRespawns          | boolean       | false      | Determines if there should be limited respawns. |
    | respawns                 | int           | 5          | How many times player can respawn during one wave. |
    | waitForWaveToEnd         | boolean       | false      | Hold wave timer until all enemies are destroyed. |
    | attackMode               | boolean       | false      | Determinates if gamemode is attack mode. |
    | editor                   | boolean       | false      | Whether this is the editor gamemode. |
    | tutorial                 | boolean       | false      | Whether the tutorial is enabled. |
    | loadout                  | [[ItemStack]] | copper:100 | Starting items put in cores |
    | bannedBlocks             | [[Block]]     |            | Blocks that cannot be placed. |
    
*** SpawnGroup
    An array of enemy spawning rule
    
    | field       | type             | default   | notes       |
    |-------------+------------------+-----------+-------------|
    | unitType    | [[UnitType]]     |           | The unit type spawned |
    | begin       | int              |           | When this spawn should start |
    | end         | int              | MAX_VALUE | When this spawn should end |
    | unitAmount  | int              | 1         | Amount of enemies spawned initially, with no scaling |
    | spacing     | int              | 1         | The spacing, in waves, of spawns. For example, 2 = spawns every other wave |
    | max         | int              | 100       | Maximum amount of units that spawn |
    | unitScaling | float            | MAX_VALUE | How many waves need to pass before the amount of units spawned increases by 1 |
    | effect      | [[StatusEffect]] |           | Status effect applied to the spawned unit. Null to disable. |
    | items       | [[ItemStack]]    |           | Items this unit spawns with. Null to disable. |
