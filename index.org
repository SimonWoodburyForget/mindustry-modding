...
** BlockStorage
   ...
   [[Abstract][Abstract]] type that extends [[Content Type][UnlockableContent]] -- This is for blocks that can store a buffer of items.
...
** Attributes
   ...
   * index ~3~ is ~oil~.
   ...
   All you need to make sprites is an image editor that supports transparency. Sprites should be ~32 x ~32 * size~, so a 2x2 block would require a ~64x64~ sprite.
