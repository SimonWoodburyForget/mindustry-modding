...
#+TEXINFO_HEADER: Modding documentation for Mindustry v100 (40)
...
** Hjson
   ...
   *[new v99]* /currently partially implimented, where only ~content/~ uses Hjson./
...
** Item
   ...
   |----------------+-----------+---------+------------|
   |                |           |         | <10>       |
   | color          | [[Color][Color]]     |         | *[required]* hex string of color  |
   | type           | [[ItemType][ItemTypes]] |         | used for tabs and core acceptance |
...
** Change Log
   ...
   :END:
   This is a log of changes done on the Mindustry Master branch. Each section is separated by tags, for example [[v100]] and then subsectioned into commit hash, for example [[968f3ace3]], with a date stamp of when said log was added, not of the commit itself. These changes typically make it into the game at the next official release.
...
**** 968f3ace3
     ...
     :END:
     [2019-11-21 Thu]
     * ~"liquid/amount"~ can now be used as a string to do the same thing as ~{ "liquid": "xxxx", "amount": xx }~ where a field type is [[LiquidStack]];
     * ~"item/amount"~ can now be used as a string to do the same thing as ~{ "item": "xxxx", "amount": xx }~ where a field type is [[ItemStack]];
     * ~mod.json~ now supports hjson;
     * ~mod.js~ can be used instead of ~mod.json~.
