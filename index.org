...
** Schematic
   ...
   Schematics can be added to your mod through the ~schematics/~ subdirectory. Schematics should have the file extension ~.msch~, but they're nothing more then base64 encoded data.
...
** Zone
   ...
   A ~Zone~ is a type that takes a map and puts it into campaign.
