...
** Bundles
   ...
   #+BEING_SRC fundamental
   block.example-mod-silver-wall.name = Серебряная Стена
   block.example-mod-silver-wall.description = Стена из серебра.
   #+END_SRC

   If you've read the first few sections of this guide, you'll spot it right away, it's simply:
   * ~<content type>.<mod name>-<content name>.name~ and,
   * ~<content type>.<mod name>-<content name>.description~.

   Where content type could be:
   * ~item~
   * ~block~
   * ~mech~
   * ~bullet~
   ...
   * ~typeid~
   ...
   Here are the file names relative to languages:
   ...
   * Chinese ? ~bundle_zh_CN.properties~ 
   * Chinese ? ~bundle_zh_TW.properties~ 
...
** Zone
   ...
   | generator            | [[Generator]]     |            | *[untested]*                                     |
   | rules                | [[Rules]]         |            | *[untested]*                                     |
...
** UNTESTED SpawnGroup
   ...
   |-------------+--------------+-----------+------------|
   |             |              |           | <10>       |
   | type        | [[UnitType]]     |           | The unit type spawned |
...
** UNTESTED Rules
   ...
   Rules is applied to map which can be edited the map editor, you can edit your mod-added campaign map's rule through these fields.
   ...
   | unitDrops                | boolean        |       true | Whether enemy units drop random items on death. |
   ...
   | respawnTime              | float          |        120 | Player respawn time in ticks. |
   | waveSpacing              | float          |       7200 | Time between waves in ticks. |
   ...
   | spawns                   | [ [[SpawnGroup]] ] |         [] | Spawn layout. |
   ...
   | tutorial                 | boolean        |      false | Whether the tutorial is enabled. False by default. |
...
** Objective
   ...
   /Trait at the base of all objective types..../


*** UNTESTED ZoneObjective
...
*** UNTESTED MapGenerator

    Extends [[Generator]] -- A basic map generator.
...
*** UNTESTED Decoration
    ...
    A method in [[MapGenerator]] -- A decoration map generator.
