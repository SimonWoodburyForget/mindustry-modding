...
* Type
  ...
  Types have noumerous fields, but the important one is ~type~. Types /extend/ each other, and what that means is if ~MissileBulletType extends BasicBulletType~, then you'll have all the fields of ~BasicBulletType~ inside of ~MissileBulletType~ like ~damage~. A ~Turret~ type, is also a ~Block~ type, so it can also have ~health~, which is a ~Block~ field.
...
** Item
   ...
   Extends [[#UnlockableContent][UnlockableContent]] -- It's the object that can ride conveyors, sorters and be stored in containers, and is commonly used in crafters.
...
** Liquid
   Extends [[#UnlockableContent][UnlockableContent]] -- Object that defines the properties of a liquid.
...
| requirements        | [[#ItemStack][ItemStack]]       | []           | Cost of constructing this block.                                                   |
...
** Environment
*** Floor
    Extends [[#Block][Block]] -- Environmental floors. Needs a sprite to be visible in the map editor.
   ...
   Sprite lookup name, where `i` is `1` or greater:
...
| weaveMag   | float |                -1 |    
** Mech
   Extends [[#UnlockableContent][UnlockableContent]] -- Mechs are the player controlled entities.
