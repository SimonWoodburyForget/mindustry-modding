...
** Item
   ...
   Extends [[#UnlockableContent][UnlockableContent]] Objects that ride conveyors/sorters and can be used in crafters.
...
Extends [[#BlockStorage][BlockStorage]] -- Attributes for all objects that are blocks.
...
*** Floor
   ...
   | walkEffect        | Effect       | "ripple"          | effect when walking on this [[#floor][floor]].          |
   | drownUpdateEffect | Effect       | "bubble"          | effect displayed when drowning on this [[#floor][floor]]. |
