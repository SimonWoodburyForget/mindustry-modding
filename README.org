...
*** ItemLiquidGenerator
    Extends [[#powergenerator][PowerGenerator]] -- Power generation block which can use items, liquids or both as input sources for power production. Liquids will take priority over items.
   ...
   Notes:
   * Doesn't account for item or liquid efficiency.
